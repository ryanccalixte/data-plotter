...
def format_stat(value):
    # convert values into display-friendly text for the stats panel
    if pd.isna(value):
        return "N/A"
...
def build_plot_stats(df, plottype):
    # build summary statistics for each numeric column in the plot data
...
def update_figure(figure, plot_stats, plottype):
    # apply layout settings and collect trace metadata for the rendered plot
...
def graphdata(cleandata, plottype):
    # create the requested chart and return the html plus summary metadata
